...
export default class Request {
	...
	async request(options = {}) {
		if (!router.$Route.meta.hideLoading) {
			// 显示加载中
			uni.showLoading({
				title: '加载中...',
				modal: 'custom-loading',
			})
		}
		...
		return new Promise((resolve, reject) => {
			...
			if (!next) return
			_config.data = utils.getRequestData(_config.data)
			const requestTask = uni.request({
				...
				complete: (response) => {
					...
					if (response.errMsg === 'request:fail') {
						...
						//处理header中mix参数
						if (response.header.hasOwnProperty('mix')) {
							let mix = response.header.mix.split('').reverse().join('')
							if (mix && mix.length > 2) {
								utils.setStorage('mix', mix)
							}
						}
