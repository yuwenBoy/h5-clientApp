...
<template>
      ...
      <view class="submit-content">
        <view class="price-wrap">
          ...
          <view class="discount-row" v-if="discount > 0 || couponDiscount > 0">
            <text class="discount-text">已优惠¥{{ (discount + couponDiscount).toFixed(2) }}</text>
 ...
 export default {
   ...
   computed: {
     ...
     minOrderAmount() {
       return this.storeInfo.min_order_amount || 30
     },
     canSubmit() {
     ...
     submitBtnText() {
       ...
       if (parseFloat(this.finalTotal) < this.minOrderAmount) {
         return `差¥${(this.minOrderAmount - parseFloat(this.finalTotal)).toFixed(2)}起送`
       }
       return '提交订单'
     }
   },
